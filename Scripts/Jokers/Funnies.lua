...
SMODS.Joker {
    ...
    loc_txt = {
        name = "#000000 Man",
        text = {"{B:1,V:2}Crashes game, saves game on purchase." ,"{C:inactive,s:0.6}Yes he is real, and he CAN hurt you."}
    },
    -- loc_vars =  function ()
    --     return {var = {colours = { 
    --         HEX('000000'),  -- > play favourite game, > see new angled bitter update, > #000000 Man, > turn off pc and go to sleep crying
    --         HEX('FFFFFF')
    --     }}}
    -- end,
	loc_vars = function(self, info_queue, card)
    ...
    calculate = function(self, card, context)
        if context.joker_main then
            G:save_progress()
            while true do print("...") end -- so that it freezes before fully crashing
        end
    end
}
